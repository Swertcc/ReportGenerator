...
def parse_judge_numbers(page_content):
    ...
    for link in soup.find_all('a', href=True):
        href = link['href']
        match = re.search(r'/judge/(\d{7})/course/116/', href)
        if match:
            number = match.group(1)
            if number.startswith('55') or number.startswith('56'):
...
def fetch_and_modify_content(url, headers, cookies):
    ...
    if location_td and location_td.find_next_sibling('td'):
        location_td.find_next_sibling('td').string = '5B-108'

    teacher_td = soup.find('td', text='指导老师')
    if teacher_td and teacher_td.find_next_sibling('td'):
        teacher_td.find_next_sibling('td').string = '虞铭财'
...
def main():
    cookies = {
        'csrftoken': 'JohVLG7Wt23ElikUO3D2PErGsTGImRqX',
        'sessionid': '4a647055cbcd71dbf680b093f9a9a2b7'
    }
    ...
    base_url = "http://acm.wzu.edu.cn/judge/course/116/judgelist/?num=50&result=AC&userprofile=29211&page="
    target_url_template = "http://acm.wzu.edu.cn/judge/{}/course/116/print_exp/"
    urls = []
    pageNum = 5
